...
export function updateURLWithState() {
    ...
    // Get the current dance style
    const style = document.body.dataset.danceType || 'salsa';
    params.set('style', style);

    // Get the active playlist
    const activePlaylistButton = document.querySelector('.btn-primary.playlist-button');
    const playlist = activePlaylistButton ? activePlaylistButton.dataset.playlist : null;
    if (playlist) params.set('playlist', playlist);

    // Get the current tag from the getter
    const activeTag = getCurrentTag();
    params.set('tag', activeTag);

    // Get autoplay and loop states
    params.set('loop', isLoopEnabled);
    if (typeof isLoopEnabled !== 'undefined') params.set('loop', isLoopEnabled);
    if (typeof isAutoplayEnabled !== 'undefined') params.set('autoplay', isAutoplayEnabled);

    // Get playback settings
    if (player?.speed !== undefined) params.set('speed', player.speed);
    if (player?.volume !== undefined) params.set('volume', player.volume);

    // Get the current video
    if (currentVideoIndex !== -1) {
        const currentMove = allMoves[currentVideoIndex];
        const video = currentMove?.video_id || currentMove?.video_filename;
    ...
    console.debug(`[URL] Updated URL to: ${newURL}`);
}
